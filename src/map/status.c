...
static int atkmods[3][MAX_WEAPON_TYPE];	// ïêäÌATKÉTÉCÉYèCê≥(size_fix.txt)
...
void status_calc_regen_rate(struct block_list *bl, struct regen_data *regen, struct status_change *sc)
{
	...
	if (
		...
		regen->flag = 0;
	if (sc->data[SC_DANCING] || sc->data[SC_OBLIVIONCURSE] || sc->data[SC_MAXIMIZEPOWER])
		//No natural SP regen
		regen->flag &=~RGN_SP;
	if (
...
int status_change_start(struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4,int tick,int flag)
{
	...
	switch(type)
	{
		...
		case SC_CONCENTRATION:
			...
			break;
		case SC_ANGELUS:
			val2 = 5*val1; //Na renovaÁ„o o aumento È na def1 enquanto antes era na def2 [Sicks]
			break;
		case SC_IMPOSITIO:
